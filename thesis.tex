...
\begin{itemize}
  ...
  \item inputs are used to connect the end switches connected in series, so if any of the switches of the given axies are closed then it goes to one pin,
...
\begin{itemize}
  ...
  \item Optimize FSM - algorithm that tries to optimize the recived power for given distance from the lens,
...
Unfortunetely in this case, used media converter dosen't provide DDM data interface, so controler must solder into SFP pins.
...
Having this information it can be concluded that with this data rate update, transcevier will be capable of compensating low frequency changes.
