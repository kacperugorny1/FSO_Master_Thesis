...
\begin{acronym}[FOUR]\itemsep=-5pt
    ...
    \acro{ISM}{Industrial, Scientific, Medical}
\end{acronym}
% ============= End Acronyms ==================
...
In a time of rapid global digitalization and great increase of need to process data, demand for high throughput and reliable communication systems keep growing.
With the expand of data rates, the need for security also rises. 
Classic RF systems, although widely used, might not satisfy needs of all customers, due to high traffic on unlicensed (ISM) bands and possible unauthorized listening.

Free Space Optical (FSO) communication might meet the requirments that RF lacks and collect the most demanding users. 
Using precisly focused light beam, this solution not only exceeds RF solutions throughput, but also provides great security, by high directivity and difficulty of hijacking the signal.

Those characteristics make the FSO systems highly competitive answer for need of private high throughput connections.
\section{Background}
...
\section{Motivation}
%The challenge of alignment and environmental interference in FSO, and the gap you are filling by using low-cost, readily available SFP modules.
...
Goal of this thesis is to design and develop a stable, high-speed FSO Ethernet link with active misalignment compensation.
This will be achieved by using low-cost, readily available SFP modules and a custom-built mechanical system for beam steering, controlled by a microcontroller that reads the DDM data from the SFP module to maintain optimal alignment.
