...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{figures/System_diagram.jpg}
...
Another two axies, y and z are parallel to the lens and the moving the source in those direction steers the beam, as shown in the figure \ref{fig5:axies}. 
...
Mechanical structure allowing movements alongside those axies has been based on lead screws and anti-backlash nut's rotated by stepper motors.
...
The shape of those guides in all axies was trapezoidal shape, as it can be seen in the figure \ref{fig6:Guides}.
...
The heart of the controller is STM32F1 microcontroller.
It's outputs are connected to the motor controllers A4988, which are configurated for 4 x microstepping.
Inputs are used to connect the end switches connected in series, so if any of the switches of the given axies are closed then it goes to one pin.
I2C bus is used to connect the microcontroller to the SFP module, to sniff out the DDM data.
There is also UART connection to the ESP32 microcontroller with PCB antenna serving as RF transceiver for communication in the link.

Block scheme is presented in the figure \ref{Fig8:Controller}.
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/Electrical_scheme.jpg}
  \caption[short]{Block scheme of controller}
